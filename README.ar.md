...
### 🌐 الوصول عن بُعد عبر الإنترنت (شبكات مختلفة، بدون سيرفر)

تريد الاتصال بهاتفك حتى وهو على **بيانات الجوال أو شبكة Wi-Fi أخرى**؟
استخدم شبكة VPN مجانية من نوع Mesh مثل **[Tailscale](https://tailscale.com)**
(الموصى به) أو ZeroTier. لا حاجة لسيرفر خاص: الأجهزة تتحدث مع بعضها مباشرة
عبر نفق WireGuard مشفّر.

1. ثبّت Tailscale على الهاتف (متجر Play) وعلى الحاسب، وسجّل الدخول بنفس
   الحساب. كل جهاز يحصل على IP دائم بصيغة `100.x.y.z`.
2. وصّل الهاتف USB مرة واحدة ونفّذ `connect` — الأداة تحفظ تلقائيًا عنوان
   Tailscale الخاص بالهاتف إلى جانب عنوان الشبكة المحلية.
3. من أي مكان في العالم: اضغط **🔄 إعادة اتصال الكل** (أو نفّذ
   `./scripts/adbconnect.sh reconnect`) — سيتصل بالعنوان المحفوظ عبر الإنترنت.

> ملاحظات: اكتشاف mDNS محلي فقط، لذا إعادة الاتصال عن بعد تعتمد على عنوان
> Tailscale المحفوظ. وبعد إعادة تشغيل الهاتف ينطفئ وضع TCP/IP — وصّل USB
> مرة واحدة لإعادة تفعيله.
